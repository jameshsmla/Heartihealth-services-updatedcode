...
public class HeartiController {
	...
	@RequestMapping(value = "/api/getTotalPredictionByPeriod/weekly", method = RequestMethod.GET, produces = {
			MediaType.APPLICATION_JSON_VALUE })

	public ResponseEntity<List<TotalPredictionWeeklyDTO>> getTotalPredictionByPeriod()
			throws SQLException {
		
		List<TotalPredictionWeeklyDTO> listOfPredictions = null;
		ResponseEntity<List<TotalPredictionWeeklyDTO>> reportDTO = null;
		HttpStatus httpStatus = null;
		
		try {
			
           listOfPredictions = heartInfoService.getTotaPredictionByWeekly();
			
			httpStatus = (listOfPredictions == null) ? HttpStatus.NO_CONTENT : HttpStatus.OK;
			
			reportDTO = new ResponseEntity<List<TotalPredictionWeeklyDTO>>(listOfPredictions, httpStatus);
	...
	@RequestMapping(value = "/api/intervalReport/weeklyReport", method = RequestMethod.GET, produces = {
			MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<List<IntervalReportWeeklyDTO>> intervalReport()
			throws SQLException {
